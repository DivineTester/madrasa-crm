...
const SubstituteWizard: React.FC<{ onFinish: (user: User) => void, onBack: () => void }> = ({ onFinish, onBack }) => {
  const { teachers, courses, settings } = useStore();
  ...
  const handlePasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (nameOption === 'select') {
      const teacher = teachers.find(t => t.fullName === selectedName);
      if (!teacher) {
    ...
    const loginId = teacher.login_id || teacher.loginId;
    if (!loginId) {
      setError('Teacher login credentials not configured');
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (result.success && result.user) {
        onFinish({
          id: `SUB-${Date.now()}`,
          name: `${selectedName} (Substitute)`,
          email: 'sub@bismillashah.com',
          role: UserRole.SUBSTITUTE,
          substituteForId: subForTeacherId,
          avatar: `https://ui-avatars.com/api/?name=${selectedName}&background=475569&color=fff`
        });
      } else {
      ...
      // Manual entry
      if (password === settings.substitutePassword) {
        onFinish({
          id: `SUB-${Date.now()}`,
        ...
        setError('Invalid substitute password');
        setIsLoading(false);
      }
    }
  };
    ...
    <div className="h-screen flex items-center justify-center bg-[#f0f9f6] p-4">
      <div className="bg-white p-6 lg:p-10 rounded-[2.5rem] shadow-2xl w-full max-w-md border-t-8 border-slate-600">
        ...
        {step === 3 && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            <div>
              <h2 className="text-xl font-black text-slate-900 tracking-tight">Step 3: Verification</h2>
              <p className="text-slate-400 text-xs mt-1">
                {nameOption === 'select'
                  ? `Enter the password for ${selectedName} to verify your identity.`
                  : 'Enter the substitute password to access the portal.'
                }
              </p>
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-emerald-500/20 outline-none font-medium pr-12"
                    value={password}
